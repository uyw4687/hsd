...
void quantize(float* input, char* quantized, int num_input, char bits_min, char bits_max, char offset, float scale)
{
  ...
  {
    quantized[i] = input[i] / scale + (float)offset; // TODO: convert floating point to quantized value
  }
}
...
const float *FPGA::blockMV(Compute* comp)
{
  ...
  if(comp->quantized)
  {
    ...
    char act_offset = (-1.0) * (float)comp->act_min / act_scale; // TODO calculate the zero-offset
    ...
    char weight_offset = (-1.0) * comp->weight_min / weight_scale; // TODO calculate the zero-offset
  ...
  for (int i = 0; i < num_output; i += m_size_)
  {
    for (int j = 0; j < num_input; j += v_size_)
    {
      ...
      for(int row = 0; row < block_row; ++row) {
      	memcpy(mat + v_size_*row, large_mat + j + num_input*(i+row), sizeof(float)*block_col);
      }
